...
saves title, alt-text, and transcript (formatted, a-z characters) as a single line
to a file named "data/xkcd_" + comic_number + ".txt" inside of data/ directory
...
def put_text_into_file(comic_number):
    ...
    save_string += transcript_string + " "
     ...
     # remove new-lines
    save_string = re.sub(r'\s+', ' ', save_string)
    # replace certain punctuation with spaces
    save_string = save_string.replace('...', ' ').replace('-', ' ').replace('—', ' ').replace('/', ' ')
    # turn all letters to lowercase
    save_string = save_string.lower()
    # turn all char that are not a-z into empty chars (includes numbers and greek letters)
    save_string = re.sub(r"[^ a-z]+", '', save_string)
    # turn multiple adjacent spaces into single space
    save_string = re.sub(' +', ' ', save_string).strip()

    print("SAVE STRING: ", save_string)

    file1 = open("data/xkcd_" + number_string + ".txt","w")
    file1.write(save_string)
    file1.close()

if __name__ == "__main__":
    """record all comics in range 0-2277. record any errors"""
    for i in tqdm(range(2277)):
        try:
            put_text_into_file(i)
        except:
            # record errors
            f = open("data/ERROR_" + str(i) + ".txt","w")
